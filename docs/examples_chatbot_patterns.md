...
- [`example_casehandler_single_turn_no_llm.py`](example_casehandler_single_turn_no_llm.py)
  - Deterministic responses (no model call, no tools).
  - Best for rule-based replies, fixed templates, simple lookups.

- [`example_casehandler_single_turn_with_llm.py`](example_casehandler_single_turn_with_llm.py)
  - One model call per inbound message.
  - Best for summarization, rewriting, classification, lightweight assistants.

- [`example_casehandler_multi_turn_tools.py`](example_casehandler_multi_turn_tools.py)
